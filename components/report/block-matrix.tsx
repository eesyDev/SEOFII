...
const PRIORITY_CONFIG = {
  must:     { label: "Нужно добавить", labelEn: "Must add",  className: "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400" },
  consider: { label: "Стоит добавить", labelEn: "Consider",  className: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400" },
  optional: { label: "Опционально", labelEn: "Optional",     className: "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400" },
} as const;
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <LayoutGrid className="h-4 w-4" /> {en ? "Which blocks to add" : "Какие блоки добавить на страницу"}
        </CardTitle>
        <p className="text-xs text-muted-foreground mt-0.5">
          {en ? "Comparison with competitors — a green ✓ means the block exists. Static analysis (no JS), may miss dynamic components." : "Сравнение с конкурентами — зелёный ✓ означает что блок есть. Анализ статический (без JS), может не видеть динамические компоненты."}
        </p>
      </CardHeader>
          ...
          <table className="w-full text-sm border-collapse min-w-[420px]">
            <thead>
              <tr className="border-b">
                <th className="text-left py-2 px-2 font-medium text-muted-foreground w-[38%]">{en ? "Block" : "Блок"}</th>
                <th className="text-center py-2 px-2 font-semibold text-primary w-[10%] border-x border-primary/20 bg-primary/5">{en ? "You" : "Вы"}</th>
                ...
                <th className="text-left py-2 px-2 font-medium text-muted-foreground">{en ? "What to do" : "Что сделать"}</th>
              </tr>
            </thead>
            <tbody>
              {missing.length > 0 && (
                  ...
                  <tr>
                    <td colSpan={3 + compCount} className="pt-3 pb-1 px-2">
                      <span className="text-xs font-semibold text-red-600 dark:text-red-400 uppercase tracking-wide">
                        {en ? "Missing — add it" : "Отсутствует — добавить"}
                      </span>
                    </td>
                  </tr>
                  {missing.map((row, i) => (
                    <BlockRow key={i} row={row} en={en} compCount={compCount} />
                  ))}
                </>
              )}
...
function BlockRow({ row, en, compCount }: { row: BlockRow; en: boolean; compCount: number }) {
    ...
    <tr className={`border-b last:border-0 ${isMissing && row.priority !== "optional" ? "bg-red-50/40 dark:bg-red-900/10" : ""}`}>
      ...
      <td className="py-2.5 px-2 align-top">
        {isMissing ? (
          ...
          <p className="text-xs text-green-600 dark:text-green-400">{en ? "Good" : "Хорошо"}</p>
        )}
      </td>
    </tr>
  );
